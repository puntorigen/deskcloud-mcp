"""
Agentic sampling loop that calls the Claude API and local implementation of tools.

Forked from Anthropic's computer-use-demo with modifications for
per-session environment isolation. Key change:
- Accepts optional pre-built ToolCollection with session-specific environment
"""
...
async def sampling_loop(
    *,
    ...
    # NEW PARAMETER: Pre-built tool collection with session environment
    tool_collection: ToolCollection | None = None,
):
    """
    Agentic sampling loop for the assistant/tool interaction of computer use.
    
    MODIFIED: Accepts optional pre-built tool_collection for per-session isolation.
    If tool_collection is provided, it will be used instead of creating new tools.
    """
    tool_group = TOOL_GROUPS_BY_VERSION[tool_version]
    
    # MODIFIED: Use provided tool_collection or create new one
    if tool_collection is None:
        # Original behavior: create tools without custom env
        tool_collection = ToolCollection(*(ToolCls() for ToolCls in tool_group.tools))
    # else: use the provided tool_collection with session-specific environment
...
def _maybe_filter_to_n_most_recent_images(
    ...
    images_to_remove -= min_removal_threshold
